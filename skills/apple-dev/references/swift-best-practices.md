...
- Protocols describing capability: suffix with `-able`, `-ing` (e.g.,
  `Equatable`, `Coding`)
- Boolean variables: use `is`, `has`, `should` prefix (e.g., `isEnabled`,
  `hasContent`)
...
- Classes for: reference semantics, inheritance, Objective-C interop, identity
  comparison
