...
description: >-
  ...
  todos in Things. Also use when the user explicitly invokes this skill,
...
metadata:
  version: "1.0.1"
  tags: "things, things3, todo, tasks, productivity, macos, url-scheme"
---
...
| --- | --- | --- |
| Interactive | Natural request or explicit skill invocation | Parse → show list → ask for edits → ask project/area → ask schedule → submit |
| Quick | User asks for quick mode or supplies `--quick` | Parse → submit to Inbox. No questions. |
...
Resolve the bundled helper relative to this `SKILL.md`, then build a JSON array
and pipe it to that absolute path:

```bash
python3 /path/to/things/scripts/things_add.py --stdin <<'JSON'
[
...
When the user asks for quick mode or passes `--quick` with the task text:
...
- Submit via the `scripts/things_add.py --stdin` helper resolved from this skill
  directory — never build `things:///` URLs inline in Bash.
