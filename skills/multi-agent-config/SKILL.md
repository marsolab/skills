...
description: "Manage portable AI coding-agent configuration for OpenAI Codex, Claude Code, and Cursor, with explicit fallbacks for unsupported mappings. Use when initializing multi-agent projects, syncing Agent Skills and project rules, translating supported MCP formats, migrating between agent platforms, or maintaining shared configuration for teams using different coding agents."
metadata:
  version: "1.0.1"
---
...
Manage and sync portable AI coding-agent configuration across OpenAI Codex,
Claude Code, and Cursor. Gemini helpers are placeholders and must not be treated
as implemented support.

All `scripts/...` paths below are relative to the directory containing this
`SKILL.md`, not the target project. Resolve the helper to an absolute path before
running it with a target project argument.
...
- OpenAI Codex (`config.toml`, `AGENTS.md`, `.agents/skills/`)
- Claude Code (`CLAUDE.md`, `.mcp.json`, `.claude/skills/`)
- Cursor (`AGENTS.md`, `.agents/skills/` or `.cursor/skills/`)

Skills and project rules sync across these targets. MCP sync is implemented for
Codex and Claude Code; Cursor MCP and all Gemini mappings require current vendor
documentation. Subagents and automatic hooks are host-specific and are not
promised as portable translations.
...
- `AGENTS.md` - Shared Codex and Cursor instructions
...
- `references/cursor.md` - Cursor skill and rule locations
...
- `references/claude-code.md` - Claude Code configuration
- `references/cursor.md` - Cursor configuration
