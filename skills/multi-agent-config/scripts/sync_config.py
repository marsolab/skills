...
class ConfigSyncer:
    ...
    GENERATED_RULES_START = "<!-- agent-config:rules:start -->"
    GENERATED_RULES_END = "<!-- agent-config:rules:end -->"
    ...
    def _write_rules(self, path: Path, rules_content: str) -> None:
        """Insert or replace the generated rules block without touching other text."""
        generated = (
            f"{self.GENERATED_RULES_START}\n"
            "# Shared Configuration\n\n"
            f"{rules_content.rstrip()}\n"
            f"{self.GENERATED_RULES_END}"
        )
        existing = path.read_text() if path.exists() else ""
        start = existing.find(self.GENERATED_RULES_START)
        end = existing.find(self.GENERATED_RULES_END)
        if start != -1 and end != -1 and start < end:
            end += len(self.GENERATED_RULES_END)
    ...
    def _sync_mcp_to_claude_code(self, servers: Dict[str, Any]) -> None:
        """Sync MCP servers to Claude Code's project .mcp.json."""
        config_file = self.project_path / '.mcp.json'
    ...
    def sync_rules(self, target_platforms: List[str]) -> None:
        ...
        # Codex and Cursor both support AGENTS.md.
        shared_agents_targets = {'codex', 'cursor'}.intersection(target_platforms)
        if shared_agents_targets:
            agents_file = self.project_path / 'AGENTS.md'
            self._write_rules(agents_file, rules_content)
            target_names = '/'.join(sorted(shared_agents_targets))
            print(f"   ✅ Synced to {target_names}: {agents_file}")
        
        # Sync to Claude Code (CLAUDE.md)
        if 'claude-code' in target_platforms:
            claude_rules = self.project_path / 'CLAUDE.md'
            self._write_rules(claude_rules, rules_content)
    ...
    def sync_skills(self, target_platforms: List[str]) -> None:
        ...
        # Codex and Cursor share the portable .agents/skills location.
        shared_skill_targets = {'codex', 'cursor'}.intersection(target_platforms)
        if shared_skill_targets:
            target = self.project_path / '.agents' / 'skills'
            ...
            target_names = '/'.join(sorted(shared_skill_targets))
            print(f"   ✅ Synced to {target_names}: {target}")
...
See the updated `~/.codex/config.toml` and project `.mcp.json` files.
...
"""
    ...
    def _update_clients_for_gateway(self) -> None:
        ...
        claude_config = self.project_path / '.mcp.json'
