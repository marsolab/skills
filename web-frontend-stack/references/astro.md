...
Astro is a web framework for content-driven websites. It renders pages to static HTML by default and only hydrates interactive components (islands) when needed. **Astro is always our foundation — React/Preact are added as islands.**
...
2. **Lazy hydration** — Use `client:idle` or `client:visible` for non-critical islands
3. **Minimal islands** — Only hydrate what needs interactivity
4. **Image optimization** — Use `<Image>` and `<Picture>` components
5. **Font loading** — Preload critical fonts, use `font-display: swap`
6. **Third-party scripts** — Isolate with Partytown
7. **CSS optimization** — Tailwind JIT, purge unused styles
